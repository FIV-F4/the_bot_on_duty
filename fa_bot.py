...
@dp.message(FAStates.waiting_for_level)
async def process_level(message: types.Message, state: FSMContext):
    ...
    if message.text not in PROBLEM_LEVELS:
        await message.answer(
            "Пожалуйста, выберите уровень из списка:",
            reply_markup=get_keyboard_from_list(PROBLEM_LEVELS)
        )
        return
...
@dp.message(FAStates.waiting_for_service)
async def process_service(message: types.Message, state: FSMContext):
    ...
    if message.text not in PROBLEM_SERVICES:
        await message.answer(
...
@dp.message(FAStates.waiting_for_naumen_type)
async def process_naumen_type(message: types.Message, state: FSMContext):
    if message.text == "❌ Отмена":
        await state.clear()
        await message.answer("Создание задачи отменено.", reply_markup=get_main_keyboard())
        return
    elif message.text == "Не заполнять":
        await state.update_data(naumen_type=None)
    elif message.text not in NAUMEN_FAILURE_TYPES:
        await message.answer(
        ...
        await state.update_data(naumen_type=message.text)
...
@dp.message(FAStates.waiting_for_stream_1c)
async def process_stream_1c(message: types.Message, state: FSMContext):
    if message.text == "❌ Отмена":
        await state.clear()
        await message.answer("Создание задачи отменено.", reply_markup=get_main_keyboard())
        return
    ...
    elif message.text not in STREAM_1C_OPTIONS:
        await message.answer(
            "Пожалуйста, выберите поток из списка:",
            reply_markup=get_keyboard_from_list(STREAM_1C_OPTIONS, is_optional=True)
        )
        return
    else:
        await state.update_data(stream_1c=message.text)
...
@dp.message(FAStates.waiting_for_influence)
async def process_influence(message: types.Message, state: FSMContext):
    if message.text == "❌ Отмена":
        await state.clear()
        await message.answer("Создание задачи отменено.", reply_markup=get_main_keyboard())
        return
    elif message.text not in INFLUENCE_OPTIONS:
        await message.answer(
            "Пожалуйста, выберите влияние из списка:",
            reply_markup=get_keyboard_from_list(INFLUENCE_OPTIONS)
        )
        return
    else:
        await state.update_data(influence=message.text)
...
@dp.message(FAStates.waiting_for_confirmation)
